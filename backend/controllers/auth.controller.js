...
export async function signup (req,res)  {
    try {
        ...
        if(!email || !password || !username){
            return res.status(400).json({Sucess:false,message:"All feilds are required"})
        }
...
export async function login (req,res)  {
    try {
		...
		const isPasswordCorrect = await bcryptjs.compare(password, user.password);    //compare the password that user enter with the password store in database in bcrypt 
